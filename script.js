...
function addToCart(event){
    ...
    <td class="uk-table-link">
        <h4 class = "item-name" style="text-align:center; padding:10px 0;">${itemName}</h4>
    </td>
    <td class="uk-text-truncate item-price"><h4 style="text-align:center; padding:10px 0;">${itemPrice}</h4></td>
    <td><input type = 'number' class = 'num' value = '1' max="10" min="1"></td>
    <td class="uk-text-truncate total-price"><h4 style="text-align:center; padding:10px 0;">${itemPrice}</h4></td>
...
function totalCost(event){
    ...
    price_field_content = price_field.innerText.replace('INR.','')
...
function grandTotal(){
    ...
    for(let i = 0; i < all_total_fields.length; i++){
        all_prices = Number(all_total_fields[i].innerText.replace('INR.',''))
        total+=all_prices
    }
